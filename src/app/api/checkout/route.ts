...
    });
    ...
    // Note: This route uses Lemon Squeezy which requires lemonSqueezyVariantId
    // Since we're using Razorpay, this route is not actively used
    // Returning error to indicate this payment method is not available
    return NextResponse.json(
      { error: "Lemon Squeezy checkout is not configured. Please use Razorpay checkout." },
      { status: 501 }
    );
    
    /* Legacy code below (commented out as lemonSqueezyVariantId field was removed)
